$(function() {
    ...
    function search(q) {
        $.ajax({
           ...
           /* success: function(response) {
                ...
                // loop through the videos array and display the title and published date
                for (var i = 0; i < videos.length; i++) {
                    console.log(videos[i].snippet.title + " - " + videos[i].snippet.publishedAt);
                }
            },
   ...
   async function play(id, title, channelTitle) {
        ...
        //$("#youtubelink").attr("value", `https://youtu.be/${id}`)
